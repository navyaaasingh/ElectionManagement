...
export default function AdminPage() {
  ...
  const updateFieldOps = useCallback((patch) => {
    setFieldOps((current) => ({ ...current, ...patch }))
  }, [])

  const saveFieldResult = useCallback((title, payload) => {
    setFieldOpsResult({ title, payload, at: new Date().toISOString() })
  }, [])

  const refreshTurnout = useCallback(async () => {
    try {
      const data = await getTurnoutMetrics()
      setTurnoutMetrics(data.turnout || data)
      saveFieldResult('Turnout refreshed', data.turnout || data)
    } catch (err) {
      alert(`Failed to refresh turnout: ${err.message}`)
    }
  }, [saveFieldResult])

  const handleStartSession = useCallback(async () => {
    try {
      if (!fieldOps.electionId || !fieldOps.boothId || !fieldOps.terminalId) {
        throw new Error('electionId, boothId and terminalId are required')
      }
      const data = await startBoothSession({
        electionId: fieldOps.electionId,
        boothId: fieldOps.boothId,
        terminalId: fieldOps.terminalId,
      })
      const nextSessionId = data.session?.session_id || data.session?.id || ''
      if (nextSessionId) {
        updateFieldOps({ sessionId: nextSessionId })
      }
      saveFieldResult('Session started', data)
    } catch (err) {
      alert(`Failed to start session: ${err.message}`)
    }
  }, [fieldOps.boothId, fieldOps.electionId, fieldOps.terminalId, saveFieldResult, updateFieldOps])

  const handleSessionAction = useCallback(async (action) => {
    try {
      if (!fieldOps.sessionId) throw new Error('sessionId is required')

      const payload = {
        reasonCode: fieldOps.reasonCode || undefined,
      }

      let data
      if (action === 'pause') data = await pauseBoothSession(fieldOps.sessionId, payload)
      if (action === 'resume') data = await resumeBoothSession(fieldOps.sessionId, payload)
      if (action === 'stop') data = await stopBoothSession(fieldOps.sessionId, payload)

      saveFieldResult(`Session ${action}`, data)
      if (action === 'stop') setQueueSnapshot(null)
    } catch (err) {
      alert(`Failed to ${action} session: ${err.message}`)
    }
  }, [fieldOps.reasonCode, fieldOps.sessionId, saveFieldResult])

  const handleLoadQueue = useCallback(async () => {
    try {
      if (!fieldOps.sessionId) throw new Error('sessionId is required')
      const data = await getBoothSessionQueue(fieldOps.sessionId)
      setQueueSnapshot(data)
      saveFieldResult('Session queue loaded', data)
    } catch (err) {
      alert(`Failed to load queue: ${err.message}`)
    }
  }, [fieldOps.sessionId, saveFieldResult])

  const handleBiometricVerification = useCallback(async () => {
    try {
      if (!fieldOps.sessionId || !fieldOps.voterId || !fieldOps.biometricTemplateHash) {
        throw new Error('sessionId, voterId and biometricTemplateHash are required')
      }
      const data = await verifyBiometric({
        sessionId: fieldOps.sessionId,
        voterId: fieldOps.voterId,
        biometricTemplateHash: fieldOps.biometricTemplateHash,
      })
      saveFieldResult('Biometric verification result', data)
    } catch (err) {
      alert(`Verification failed: ${err.message}`)
    }
  }, [fieldOps.biometricTemplateHash, fieldOps.sessionId, fieldOps.voterId, saveFieldResult])

  const handleCreateOverride = useCallback(async () => {
    try {
      if (!fieldOps.sessionId || !fieldOps.voterId || !fieldOps.manualReasonCode) {
        throw new Error('sessionId, voterId and reasonCode are required')
      }
      const data = await createManualOverride({
        sessionId: fieldOps.sessionId,
        voterId: fieldOps.voterId,
        reasonCode: fieldOps.manualReasonCode,
        notes: fieldOps.manualNotes || undefined,
      })

      const overrideRequestId = data.data?.overrideRequestId || data.overrideRequestId
      if (overrideRequestId) {
        updateFieldOps({ overrideRequestId: String(overrideRequestId) })
      }
      saveFieldResult('Manual override request created', data)
    } catch (err) {
      alert(`Override request failed: ${err.message}`)
    }
  }, [fieldOps.manualNotes, fieldOps.manualReasonCode, fieldOps.sessionId, fieldOps.voterId, saveFieldResult, updateFieldOps])

  const handleResolveOverride = useCallback(async () => {
    try {
      if (!fieldOps.overrideRequestId) throw new Error('overrideRequestId is required')
      const data = await resolveManualOverride(fieldOps.overrideRequestId, {
        decision: fieldOps.overrideDecision,
        notes: fieldOps.overrideNotes || undefined,
      })
      saveFieldResult('Manual override resolved', data)
    } catch (err) {
      alert(`Override resolve failed: ${err.message}`)
    }
  }, [fieldOps.overrideDecision, fieldOps.overrideNotes, fieldOps.overrideRequestId, saveFieldResult])

  const handleIssueBallotToken = useCallback(async () => {
    try {
      if (!fieldOps.sessionId || !fieldOps.voterId) throw new Error('sessionId and voterId are required')
      const data = await issueBallotToken({
        sessionId: fieldOps.sessionId,
        voterId: fieldOps.voterId,
        ttlSeconds: Number(fieldOps.ttlSeconds) || 120,
      })
      saveFieldResult('Ballot token issued', data)
    } catch (err) {
      alert(`Ballot issue failed: ${err.message}`)
    }
  }, [fieldOps.sessionId, fieldOps.ttlSeconds, fieldOps.voterId, saveFieldResult])

  const handleLogCustodyEvent = useCallback(async () => {
    try {
      if (!fieldOps.electionId || !fieldOps.custodyEventType) {
        throw new Error('electionId and custody event type are required')
      }

      let payload = {}
      if (fieldOps.custodyPayload?.trim()) {
        payload = JSON.parse(fieldOps.custodyPayload)
      }

      const data = await createCustodyEvent({
        electionId: fieldOps.electionId,
        boothId: fieldOps.boothId || undefined,
        terminalId: fieldOps.terminalId || undefined,
        eventType: fieldOps.custodyEventType,
        payload,
      })
      saveFieldResult('Custody event logged', data)
    } catch (err) {
      alert(`Failed to log custody event: ${err.message}`)
    }
  }, [fieldOps.boothId, fieldOps.custodyEventType, fieldOps.custodyPayload, fieldOps.electionId, fieldOps.terminalId, saveFieldResult])

  const handleLoadCustody = useCallback(async () => {
    try {
      const data = await getCustodyEvents({
        electionId: fieldOps.electionId || undefined,
        boothId: fieldOps.boothId || undefined,
        terminalId: fieldOps.terminalId || undefined,
        from: fieldOps.from || undefined,
        to: fieldOps.to || undefined,
        limit: 30,
      })
      const events = data.data?.events || []
      setCustodySnapshot(events)
      saveFieldResult('Custody chain loaded', data)
    } catch (err) {
      alert(`Failed to load custody events: ${err.message}`)
    }
  }, [fieldOps.boothId, fieldOps.electionId, fieldOps.from, fieldOps.terminalId, fieldOps.to, saveFieldResult])

  const handleLoadPolicy = useCallback(async () => {
    try {
      if (!fieldOps.electionId) throw new Error('electionId is required')
      const data = await getEligibilityPolicy(fieldOps.electionId)
      const rules = data.data?.rules || {}
      updateFieldOps({
        policyId: data.data?.policyId || fieldOps.policyId,
        policyName: data.data?.metadata?.name || fieldOps.policyName,
        policyDescription: data.data?.metadata?.description || fieldOps.policyDescription,
        policyRulesJson: JSON.stringify(rules, null, 2),
      })
      saveFieldResult('Eligibility policy loaded', data)
    } catch (err) {
      alert(`Failed to load eligibility policy: ${err.message}`)
    }
  }, [fieldOps.electionId, fieldOps.policyDescription, fieldOps.policyId, fieldOps.policyName, saveFieldResult, updateFieldOps])

  const handleSavePolicy = useCallback(async () => {
    try {
      if (!fieldOps.electionId) throw new Error('electionId is required')
      const parsedRules = JSON.parse(fieldOps.policyRulesJson || '{}')
      if (parsedRules.__meta) delete parsedRules.__meta

      const payload = {
        electionId: fieldOps.electionId,
        name: fieldOps.policyName,
        description: fieldOps.policyDescription,
        versionNote: fieldOps.policyVersionNote,
        rules: parsedRules,
      }

      const data = fieldOps.policyId
        ? await updateEligibilityPolicy(fieldOps.policyId, payload)
        : await createEligibilityPolicy(payload)

      if (data.data?.policyId) {
        updateFieldOps({ policyId: data.data.policyId })
      }
      saveFieldResult('Eligibility policy saved', data)
    } catch (err) {
      alert(`Failed to save policy: ${err.message}`)
    }
  }, [fieldOps.electionId, fieldOps.policyDescription, fieldOps.policyId, fieldOps.policyName, fieldOps.policyRulesJson, fieldOps.policyVersionNote, saveFieldResult, updateFieldOps])

  const handleEligibilityBulkValidate = useCallback(async () => {
    try {
      if (!fieldOps.electionId) throw new Error('electionId is required')

      const voterIds = String(fieldOps.voterIdsCsv || '')
        .split(/[\s,]+/)
      ...
      if (voterIds.length > 0) payload.voterIds = voterIds

      const data = await bulkValidateEligibility(payload)
      setEligibilitySnapshot(data.data || null)
      saveFieldResult('Eligibility bulk validation completed', data)
    } catch (err) {
      alert(`Eligibility validation failed: ${err.message}`)
    }
  }, [fieldOps.electionId, fieldOps.voterIdsCsv, saveFieldResult])
          ...
          >
            ...
            <div className="surface-card" style={{ marginTop: '24px', padding: '24px' }}>
              ...
              <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                ...
                <AnimatedButton className="button button--ghost" onClick={refreshTurnout}>Refresh Turnout</AnimatedButton>
                {simulationResult && (
              ...
              {turnoutMetrics && (
                <p style={{ marginTop: '12px', fontSize: '0.9rem', color: 'var(--ink-soft)' }}>
                  Live turnout: <strong>{Number(turnoutMetrics.turnoutPct || 0).toFixed(2)}%</strong> | Registered: <strong>{turnoutMetrics.registeredVoters || 0}</strong> | Votes cast: <strong>{turnoutMetrics.votesCast || 0}</strong>
                </p>
              )}
            </div>
          </motion.div>
        ...
        >
          <div className="workspace-grid">
            <div className="surface-card" style={{ padding: '24px' }}>
              <div className="section-heading section-heading--compact" style={{ marginBottom: '16px' }}>
                <p className="section-kicker">Supervisor console</p>
              ...
              <div className="field-grid" style={{ marginBottom: '12px' }}>
                <label>
                  ...
                  <select
                    className="field-input"
                    value={fieldOps.electionId}
                    onChange={(e) => updateFieldOps({ electionId: e.target.value })}
                  >
                ...
                <label>
                  <span className="field-label">Booth ID</span>
                  <input
                    className="field-input"
                    value={fieldOps.boothId}
                    onChange={(e) => updateFieldOps({ boothId: e.target.value })}
                    placeholder="Polling booth UUID"
                  />
                </label>
                <label>
                  <span className="field-label">Terminal ID</span>
                  <input
                    className="field-input"
                    value={fieldOps.terminalId}
                    onChange={(e) => updateFieldOps({ terminalId: e.target.value })}
                    placeholder="TERM-WEB-001"
                  />
                </label>
                <label>
                  ...
                  <input
                    className="field-input"
                    value={fieldOps.sessionId}
                    onChange={(e) => updateFieldOps({ sessionId: e.target.value })}
                    placeholder="Auto-populated after start"
                  />
                </label>
              </div>
              ...
              <div className="field-grid" style={{ marginBottom: '14px' }}>
                <label>
                  ...
                  <input
                    className="field-input"
                    value={fieldOps.reasonCode}
                    onChange={(e) => updateFieldOps({ reasonCode: e.target.value })}
                    placeholder="SECURITY_CHECK"
                  />
                </label>
              </div>

              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                <AnimatedButton className="button button--primary button--inline" onClick={handleStartSession}>Start</AnimatedButton>
                <AnimatedButton className="button button--ghost button--inline" onClick={() => handleSessionAction('pause')}>Pause</AnimatedButton>
                <AnimatedButton className="button button--ghost button--inline" onClick={() => handleSessionAction('resume')}>Resume</AnimatedButton>
                <AnimatedButton className="button button--ghost button--inline" onClick={() => handleSessionAction('stop')}>Stop</AnimatedButton>
                <AnimatedButton className="button button--ghost button--inline" onClick={handleLoadQueue}>Load Queue</AnimatedButton>
              </div>

              {queueSnapshot && (
                <div style={{ marginTop: '16px', fontSize: '0.9rem', color: 'var(--ink-soft)' }}>
                  Pending overrides: <strong>{queueSnapshot.pendingOverrides || 0}</strong> | Active tokens: <strong>{queueSnapshot.activeTokens || 0}</strong> | Attempts: <strong>{queueSnapshot.attempts?.length || 0}</strong>
                </div>
              )}
            </div>
            ...
            <div className="surface-card" style={{ padding: '24px' }}>
              <div className="section-heading section-heading--compact" style={{ marginBottom: '16px' }}>
                ...
                <h2>Verify, override, issue ballot</h2>
              </div>

              <div className="field-grid" style={{ marginBottom: '12px' }}>
                <label>
                  <span className="field-label">Voter ID</span>
                  <input
                    className="field-input"
                    value={fieldOps.voterId}
                    onChange={(e) => updateFieldOps({ voterId: e.target.value })}
                    placeholder="Voter UUID"
                  />
                </label>
                <label>
                  <span className="field-label">Biometric SHA-256 hash</span>
                  <input
                    className="field-input"
                    value={fieldOps.biometricTemplateHash}
                    onChange={(e) => updateFieldOps({ biometricTemplateHash: e.target.value })}
              ...
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '16px' }}>
                <AnimatedButton className="button button--primary button--inline" onClick={handleBiometricVerification}>Verify Biometric</AnimatedButton>
                <AnimatedButton className="button button--ghost button--inline" onClick={handleIssueBallotToken}>Issue Ballot Token</AnimatedButton>
              </div>

              <div className="field-grid" style={{ marginBottom: '12px' }}>
                <label>
                  <span className="field-label">Manual reason code</span>
                  <input
                    className="field-input"
                    value={fieldOps.manualReasonCode}
                    onChange={(e) => updateFieldOps({ manualReasonCode: e.target.value })}
                    placeholder="BIOMETRIC_SENSOR_FAILURE"
                  />
                </label>
                <label>
                  <span className="field-label">Token TTL (seconds)</span>
                  <input
                    className="field-input"
                    type="number"
                    value={fieldOps.ttlSeconds}
                    onChange={(e) => updateFieldOps({ ttlSeconds: Number(e.target.value) })}
                  />
                </label>
              </div>

              <label>
                <span className="field-label">Manual notes</span>
                <input
                  className="field-input"
                  value={fieldOps.manualNotes}
                  onChange={(e) => updateFieldOps({ manualNotes: e.target.value })}
                  placeholder="Why override is needed"
                />
              </label>

              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginTop: '10px' }}>
                <AnimatedButton className="button button--ghost button--inline" onClick={handleCreateOverride}>Create Override</AnimatedButton>
              </div>

              <div className="field-grid" style={{ marginTop: '12px' }}>
                <label>
                  <span className="field-label">Override Request ID</span>
                  <input
                    className="field-input"
                    value={fieldOps.overrideRequestId}
                    onChange={(e) => updateFieldOps({ overrideRequestId: e.target.value })}
                ...
                <label>
                  ...
                  <select
                    className="field-input"
                    value={fieldOps.overrideDecision}
                    onChange={(e) => updateFieldOps({ overrideDecision: e.target.value })}
                  >
              ...
              <label style={{ marginTop: '10px', display: 'block' }}>
                ...
                <input
                  className="field-input"
                  value={fieldOps.overrideNotes}
                  onChange={(e) => updateFieldOps({ overrideNotes: e.target.value })}
                />
              </label>

              <div style={{ display: 'flex', gap: '8px', marginTop: '10px' }}>
                <AnimatedButton className="button button--primary button--inline" onClick={handleResolveOverride}>Resolve Override</AnimatedButton>
              </div>
            </div>
          </div>
          ...
          <div className="workspace-grid">
            <div className="surface-card" style={{ padding: '24px' }}>
              <div className="section-heading section-heading--compact" style={{ marginBottom: '16px' }}>
                <p className="section-kicker">Custody chain</p>
                <h2>Event ledger controls</h2>
              </div>
              ...
              <div className="field-grid" style={{ marginBottom: '12px' }}>
                <label>
                  ...
                  <select
                    className="field-input"
                    value={fieldOps.custodyEventType}
                    onChange={(e) => updateFieldOps({ custodyEventType: e.target.value })}
                  >
                    {[
                      'DEVICE_HANDOVER',
                      'DEVICE_SEAL_APPLIED',
                ...
                <label>
                  ...
                  <input
                    ...
                    type="datetime-local"
                    value={fieldOps.from}
                    onChange={(e) => updateFieldOps({ from: e.target.value })}
                  />
                </label>
                <label>
                  ...
                  <input
                    ...
                    type="datetime-local"
                    value={fieldOps.to}
                    onChange={(e) => updateFieldOps({ to: e.target.value })}
                  />
                </label>
              </div>
              ...
              <label>
                ...
                <textarea
                  className="field-input"
                  style={{ width: '100%', minHeight: '120px', fontFamily: 'monospace', fontSize: '0.85rem' }}
                  value={fieldOps.custodyPayload}
                  onChange={(e) => updateFieldOps({ custodyPayload: e.target.value })}
                />
              </label>

              <div style={{ display: 'flex', gap: '8px', marginTop: '10px', flexWrap: 'wrap' }}>
                <AnimatedButton className="button button--primary button--inline" onClick={handleLogCustodyEvent}>Log Event</AnimatedButton>
                <AnimatedButton className="button button--ghost button--inline" onClick={handleLoadCustody}>Load Chain</AnimatedButton>
              </div>

              <div style={{ marginTop: '14px', maxHeight: '220px', overflowY: 'auto', borderTop: '1px solid var(--line-soft)', paddingTop: '12px' }}>
                {custodySnapshot.length === 0 ? (
                  <p style={{ margin: 0, color: 'var(--ink-muted)' }}>No custody events loaded.</p>
                ) : custodySnapshot.map((event) => (
                  <article key={event.custody_event_id || event.event_hash} style={{ padding: '8px 0', borderBottom: '1px solid var(--line-soft)' }}>
                    <strong style={{ fontSize: '0.85rem' }}>{event.event_type}</strong>
                    <p style={{ margin: '4px 0 0', fontSize: '0.8rem', color: 'var(--ink-soft)' }}>
                      {new Date(event.created_at).toLocaleString()} | hash {String(event.event_hash || '').slice(0, 12)}...
                    </p>
                  </article>
                ))}
              </div>
            ...
            <div className="surface-card" style={{ padding: '24px' }}>
              <div className="section-heading section-heading--compact" style={{ marginBottom: '16px' }}>
                <p className="section-kicker">Eligibility controls</p>
                <h2>Policy + bulk validation</h2>
              </div>

              <div className="field-grid" style={{ marginBottom: '10px' }}>
                <label>
                  <span className="field-label">Policy ID</span>
                  <input className="field-input" value={fieldOps.policyId} onChange={(e) => updateFieldOps({ policyId: e.target.value })} placeholder="optional" />
                </label>
                <label>
                  <span className="field-label">Policy name</span>
                  <input className="field-input" value={fieldOps.policyName} onChange={(e) => updateFieldOps({ policyName: e.target.value })} />
                </label>
              </div>
              ...
              <div className="field-grid" style={{ marginBottom: '10px' }}>
                <label>
                  ...
                  <input className="field-input" value={fieldOps.policyDescription} onChange={(e) => updateFieldOps({ policyDescription: e.target.value })} />
                </label>
                <label>
                  <span className="field-label">Version note</span>
                  <input className="field-input" value={fieldOps.policyVersionNote} onChange={(e) => updateFieldOps({ policyVersionNote: e.target.value })} />
                </label>
              </div>

              <label>
                <span className="field-label">Policy rules JSON</span>
                <textarea
                  className="field-input"
                  style={{ width: '100%', minHeight: '130px', fontFamily: 'monospace', fontSize: '0.85rem' }}
                  value={fieldOps.policyRulesJson}
                  onChange={(e) => updateFieldOps({ policyRulesJson: e.target.value })}
                />
              </label>

              <div style={{ display: 'flex', gap: '8px', marginTop: '10px', flexWrap: 'wrap' }}>
                <AnimatedButton className="button button--ghost button--inline" onClick={handleLoadPolicy}>Load Policy</AnimatedButton>
                <AnimatedButton className="button button--primary button--inline" onClick={handleSavePolicy}>Save Policy</AnimatedButton>
              </div>

              <label style={{ display: 'block', marginTop: '12px' }}>
                <span className="field-label">Voter IDs (comma or space separated)</span>
                <textarea
                  className="field-input"
                  style={{ width: '100%', minHeight: '72px' }}
                  value={fieldOps.voterIdsCsv}
                  onChange={(e) => updateFieldOps({ voterIdsCsv: e.target.value })}
                  placeholder="Leave blank to validate first 200 voters"
                />
              </label>

              <div style={{ display: 'flex', gap: '8px', marginTop: '10px' }}>
                <AnimatedButton className="button button--ghost button--inline" onClick={handleEligibilityBulkValidate}>Run Bulk Validate</AnimatedButton>
              </div>

              {eligibilitySnapshot && (
                <p style={{ marginTop: '12px', fontSize: '0.9rem', color: 'var(--ink-soft)' }}>
                  Evaluated: <strong>{eligibilitySnapshot.totalEvaluated || 0}</strong> | Eligible: <strong>{eligibilitySnapshot.eligible || 0}</strong> | Ineligible: <strong>{eligibilitySnapshot.ineligible || 0}</strong>
                </p>
              )}
            </div>
          </div>
          ...
          <div className="surface-card" style={{ padding: '24px' }}>
            <div className="section-heading section-heading--compact" style={{ marginBottom: '10px' }}>
              <p className="section-kicker">Latest response</p>
              <h2>Field ops output</h2>
            </div>
            {fieldOpsResult ? (
              <>
                <p style={{ margin: '0 0 10px', fontSize: '0.85rem', color: 'var(--ink-muted)' }}>
                  {fieldOpsResult.title} · {new Date(fieldOpsResult.at).toLocaleTimeString()}
                </p>
                <pre style={{ margin: 0, whiteSpace: 'pre-wrap', wordBreak: 'break-word', fontSize: '0.8rem', fontFamily: 'monospace', background: 'var(--surface-sunken)', border: '1px solid var(--line-soft)', padding: '12px', borderRadius: '10px' }}>
                  {JSON.stringify(fieldOpsResult.payload, null, 2)}
                </pre>
              </>
            ) : (
              <p style={{ margin: 0, color: 'var(--ink-muted)' }}>Run a field operation to preview live API payloads here.</p>
            )}
          </div>
        </motion.div>
      ) : null}
