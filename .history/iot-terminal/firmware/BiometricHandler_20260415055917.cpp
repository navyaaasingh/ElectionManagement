...
bool BiometricHandler::begin() {
  ...
  if (finger->verifyPassword()) {
    ...
    uint8_t packetConfigResult = finger->setPacketSize(FINGERPRINT_PACKET_SIZE_64);
    if (packetConfigResult == FINGERPRINT_OK) {
      Serial.println("   Packet size set to 64 bytes");
    } else {
      Serial.print("   ⚠️ Failed to set packet size, code: ");
      Serial.println(packetConfigResult);
    }
...
String BiometricHandler::getTemplateHash() {
  ...
  for (int attempt = 0; attempt < 2; attempt++) {
    ...
    if (attempt == 0) {
      Serial.println("⚠️ Template stream failed, retrying with packet size 64");
