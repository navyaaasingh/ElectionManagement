    ...
    try {
        ...
        if (req.user.role === 'admin' && req.user.adminRole !== 'SUPER_ADMIN') {
            ...
            if (!election || election.created_by_admin_id !== req.user.adminId) {
                return res.status(403).json({ success: false, error: 'You can only manage sessions for your elections' });
            }
        }
...
router.get('/sessions/:sessionId/queue', authenticate, authorize('admin', 'supervisor'), async (req, res) => {
    try {
        ...
        if (req.user.role === 'admin' && req.user.adminRole !== 'SUPER_ADMIN') {
            const election = await Election.findByPk(session.election_id);
            if (!election || election.created_by_admin_id !== req.user.adminId) {
                return res.status(403).json({ success: false, error: 'You can only view queues for your elections' });
            }
        }
