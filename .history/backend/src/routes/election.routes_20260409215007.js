...
const ELECTION_TYPE_MAP = {
    ...
    BYELECTION: 'INSTITUTIONAL',
    BY_ELECTION: 'INSTITUTIONAL',
    'BY-ELECTION': 'INSTITUTIONAL',
    'SPECIAL/BY-ELECTION': 'INSTITUTIONAL',
    SPECIAL_BY_ELECTION: 'INSTITUTIONAL',
};
...
router.post('/', authenticate, authorize('admin'), csrfProtection, async (req, res) => {
    try {
        ...
        const eligibilityRules = parseJsonField(req.body.eligibilityRules ?? req.body.eligibility_rules);
        const runoffConfig = parseJsonField(req.body.runoffConfig ?? req.body.runoff_config);
...
router.put('/:id', authenticate, authorize('admin'), csrfProtection, async (req, res) => {
    try {
        ...
        if (req.body.eligibilityRules !== undefined || req.body.eligibility_rules !== undefined) {
        ...
        if (req.body.runoffConfig !== undefined || req.body.runoff_config !== undefined) {
            const parsed = parseJsonField(req.body.runoffConfig ?? req.body.runoff_config);
            if (parsed === undefined) {
                return res.status(400).json({ success: false, error: 'Invalid runoffConfig JSON' });
            }
            updateData.runoff_config = parsed;
        }
