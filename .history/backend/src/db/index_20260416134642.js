...
if (forceSqlite || (process.env.NODE_ENV === 'development' && !process.env.POSTGRES_HOST)) {
...
const ensureSchemaCompatibility = async () => {
    ...
    await ensureColumn('voters', 'is_biometric_registered', {
        type: Sequelize.BOOLEAN,
        allowNull: false,
        defaultValue: false,
    });
    await ensureColumn('voters', 'aadhaar_verified', {
        type: Sequelize.BOOLEAN,
        allowNull: false,
        defaultValue: false,
    });
    await ensureColumn('voters', 'device_id', {
        type: Sequelize.STRING(255),
        allowNull: true,
    });
    ...
    await ensureColumn('students', 'section', {
        type: Sequelize.STRING(20),
        allowNull: true,
    });
    await ensureColumn('students', 'class_name', {
        type: Sequelize.STRING(50),
        allowNull: true,
    });
    await ensureColumn('students', 'academic_year', {
        type: Sequelize.INTEGER,
        allowNull: true,
    });
    await ensureColumn('students', 'semester', {
        type: Sequelize.INTEGER,
        allowNull: true,
    });
    await ensureColumn('voters', 'section', {
        type: Sequelize.STRING(20),
        allowNull: true,
    });
    await ensureColumn('voters', 'class_name', {
        type: Sequelize.STRING(50),
        allowNull: true,
    });
    await ensureColumn('voters', 'academic_year', {
        type: Sequelize.INTEGER,
        allowNull: true,
    });
    ...
    if (dialect === 'postgres') {
        try {
            await sequelize.query(
                "UPDATE voters SET status = LOWER(status) WHERE status IS NOT NULL;"
            );
            await sequelize.query(
                "UPDATE voters SET status = 'pending' WHERE status IS NULL OR status NOT IN ('pending', 'active', 'suspended', 'deceased');"
            );
            await sequelize.query(`
                DO $$
                BEGIN
                    IF EXISTS (
                        SELECT 1
                        FROM pg_constraint
                        WHERE conname = 'voters_status_check'
                    ) THEN
                        ALTER TABLE voters DROP CONSTRAINT voters_status_check;
                    END IF;
                END
                $$;
            `);
            await sequelize.query(`
                ALTER TABLE voters
                ADD CONSTRAINT voters_status_check
                CHECK (status IN ('pending', 'active', 'suspended', 'deceased'));
            `);
            await sequelize.query(
                "ALTER TABLE voters ALTER COLUMN status SET DEFAULT 'pending';"
            );
            console.log('✅ Ensured voters status compatibility for pending approval workflow');
        } catch (error) {
            console.warn('⚠️ Could not normalize voters status constraint:', error.message);
        }
    }
    ...
    if (dialect === 'postgres') {
        const schemaToTables = {
            ...
            [CONTEXT_SCHEMAS.election]: [
                ...
                'booth_device_assignments',
            ],
            [CONTEXT_SCHEMAS.vote]: [
                'voting_records',
                'vote_nonces',
                'outbox_events',
                'dead_letter_events',
        ...
        const publicTableExists = async (tableName) => {
            try {
        ...
        for (const [schemaName, tables] of Object.entries(schemaToTables)) {
            ...
            for (const tableName of tables) {
                if (!(await publicTableExists(tableName))) {
                    console.warn(`⚠️ Skipping context view ${schemaName}.${tableName} because public.${tableName} does not exist yet`);
                    continue;
                }
